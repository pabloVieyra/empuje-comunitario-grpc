...
const EditFilterModal = ({ filter, onClose, onEdit }) => {
  const [name, setName] = useState(filter.name);

  // Si quieres permitir editar el filtro, deberías agregar más inputs aquí.
  return (
    <Box
      position="fixed"
      top="0" left="0"
    ...
    >
      <Card style={{
        ...
        <Flex gap={4} justify="center">
          <Btn onClick={() => {
            // Pasa todos los datos necesarios, incl. name y filter
            onEdit({ ...filter, name });
            onClose();
